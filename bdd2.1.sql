CREATE DATABASE  IF NOT EXISTS `proxibanquebdd` /*!40100 DEFAULT CHARACTER SET utf16 */;
USE `proxibanquebdd`;
...
CREATE TABLE `client` (
  ...
  CONSTRAINT `fk_conseiller` FOREIGN KEY (`idConseiller`) REFERENCES `conseiller` (`idConseiller`) ON DELETE NO ACTION ON UPDATE NO ACTION
...
CREATE TABLE `compte` (
  ...
  CONSTRAINT `fk_type` FOREIGN KEY (`idTypeCompte`) REFERENCES `typecompte` (`idTypeCompte`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf16;
/*!40101 SET character_set_client = @saved_cs_client */;
...
-- Dump completed on 2018-04-12 12:54:19
